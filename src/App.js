...
const addPercentagesToLabels = (data = []) => {
  return [data[0]].concat(data.slice(1).map((item, index) => ({
    ...item,
    name: `${item.name} (${data[index].value <= 0 ? 0 : Math.round(item.value / data[index].value * 100)}%)`
  })))
};
...
class App extends React.Component {
  ...
  timer = () => !this.state.timer || this.state.timer < 0 ? 0 : this.state.timer
  timerProgress = () => this.timer() / AUTO_REFRESH_RATE_SECS * 100

  cachedOrNewResponse = (boardIds = []) => this.state.previousResponse.data ?
  ...
  updateCache = (boardIds = []) =>
    monday
      .api(QUERY, { variables: { boardIds } })
      .then((res) => { this.setState({ previousResponse: res }) })
  ...
  apiCall = () => {
    ...
    return this.cachedOrNewResponse(this.boardIds())
      .then((res) => {
        if (this.labelColumn() !== NAME_COLUMN &&
          !res.data.boards.flatMap(({ items }) => items).flatMap(({ column_values }) => column_values).find(({ id }) => id === this.labelColumn())) {
          throw new Error(LABEL_ERROR);
        }
        if (!res.data.boards.flatMap(({ items }) => items).flatMap(({ column_values }) => column_values).find(({ id }) => id === this.valueColumn())) {
          throw new Error(VALUE_ERROR);
        }

        const data =
          res.data.boards.flatMap(({ items }) => items).map(({ name, column_values }) => ({
            name:
              this.labelColumn() === NAME_COLUMN ?
                name :
                column_values.find(({ id }) => id === this.labelColumn())?.text || "",
            value:
              Number(column_values.find(({ id }) => id === this.valueColumn())?.text || ""),
          }))
        this.setState({
  ...
  refreshButtonHandler = () => {
    this.setState({ disabled: true })
    this.updateCache(this.boardIds())
      .then(() => this.apiCall())
  };

  displayAlert = () => {
    if (!this.boardIds().length) {
      return <Alert variant="info" className="mt-4">Please select at least 1 board</Alert>;
    } else if (this.labelColumn() === "") {
      return <Alert variant="info" className="mt-4">Please select a label column</Alert>;
    } else if (this.valueColumn() === "") {
      return <Alert variant="info" className="mt-4">Please select a value column</Alert>;
      ...
      <div className="container">
        {this.displayAlert() || (
          <div className="funnel-container pt-4">
            ...
            <div className="pt-4">
              <Button variant="success" size="lg" disabled={this.state.disabled ? true : false} onClick={this.refreshButtonHandler}>
                {this.state.disabled ? <Spinner animation="border" variant="default" /> : "Refresh"}
              </Button>
              <div className="mt-1">
